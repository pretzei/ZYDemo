...
@interface WIBrushView ()

// 点集合
@property (nonatomic, strong) NSArray *points;
//重力感应
@property (nonatomic, assign) CGFloat touchForce;
// 当前宽度
...
//        self.debug = YES;
    ...
    if (self.debug) {
        ...
        self.points = @[value1,value2,value3];
...
- (BOOL)open3DTouch {
    return [self viewController].traitCollection.forceTouchCapability == UIForceTouchCapabilityAvailable;
}
...
- (void)changeImage {
    ...
    if (len == 0) {
        ...
        CGFloat aimWidth = (self.open3DTouch ? self.touchForce /2 + 150.0/(CGFloat)len : 300.0/(CGFloat)len) * (kWIDTH_MAX - kWIDTH_MIN);
        // 获取贝塞尔点集
